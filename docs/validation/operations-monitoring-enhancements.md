...
- 管理页面将“当前实际值”和“预警/严重告警线”分开显示。例如“失败任务达到 1 个预警、5 个严重”是阈值，实际失败数单独显示；磁盘空间也明确标注当前已用、可用和总量。
- 健康问题可直接定位到相关任务。处理队列支持“需要关注、失败、排队、处理中、全部”筛选；失败项可展开完整错误原因，并显示任务编号、来源、更新时间、尝试次数和人工重新排队入口。
...
- 确认告警线与当前值没有用斜线混排；点击后台任务健康问题可定位到对应筛选，失败任务可展开完整原因。
- 在 390px 窄屏检查运维面板、告警线和任务筛选不产生页面横向滚动。
