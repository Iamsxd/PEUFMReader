...
- `pnpm test:e2e:list` 已发现桌面与移动端共 8 个场景。
- 前端类型检查、30 个组件/逻辑测试及生产 Docker 构建已通过。
- 2026-07-22 尝试连接应用内真实浏览器时，浏览器插件初始化报错 `Cannot redefine property: process`；因此本次未声明 8 个真实浏览器场景通过。修复浏览器插件运行时后，应按上面的命令补跑并检查截图、视频和 trace。
