...
已完成并有记录的内容包括：P1 移动离线阅读/PWA/前端加载优化；P1.5 目录查询优化；P2.1 基础运维监控；P2.4 第一版独立阅读统计页面。最近提交修复了离线保存的 EPUB 按二进制方式打开的问题。

后续路线在 `docs/product/development-roadmap.md`，但不要自行假定优先级。P2 的公开候选方向是：运维告警与外部指标、跨书库全文检索与 AI、听书、阅读目标/导出/隐私等统计增强。开始新功能前先向项目负责人确认本次目标。
