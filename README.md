...
在 Unraid 终端从项目目录执行 `sh scripts/backup.sh`。脚本会短暂停止应用写入，导出 PostgreSQL，并打包书库、缓存和导入目录；校验文件保存在 `${PEUFM_BACKUP_ROOT}` 下。可传入快照名，例如 `sh scripts/backup.sh before-upgrade`。

恢复会替换当前数据库和托管文件，必须明确确认：`sh scripts/restore.sh before-upgrade --yes`。恢复前建议先额外复制当前数据目录；脚本校验备份完整性后才会清理目标目录，并在结束或失败时重新启动应用。

3000 本书、10 并发用户的可重复 API 基线脚本位于 `scripts/performance-seed.sql` 和 `scripts/performance-smoke.mjs`，本次实测结果与限制见 [M2 验证记录](docs/validation/m2-reader-import-operations-validation.md)。
