...
- 上传前按大小预筛选、本地分块 SHA-256 确认重复，跳过重复文件传输；服务端保留最终去重和真实格式签名校验。
...
- 首页框架和搜索栏立即显示；继续阅读、个人统计、最近加入优先加载，分类、热门和推荐随后独立补齐，单个慢查询不会阻塞整页。
- 独立书籍详情页、收藏书架和多信号个性化推荐；推荐显示作者、题材、热度等原因，并支持“感兴趣/不感兴趣”反馈。
- 服务端全文条件搜索、分页、排序和分类筛选。
- 管理员可批量修改版本元数据、检查同名/同 ISBN 的疑似重复项并合并作品或版本；完全相同文件会按 SHA-256 在导入时阻止重复入库。
- 管理员可删除书籍：应用托管副本会连同本应用中的阅读记录移除，但上传、收件箱和监控来源文件保持不动；Calibre 只读引用只移除本应用索引，绝不删除 Calibre 原书。
- 待整理书目采用服务端分页、搜索和格式/原因筛选；列表只加载摘要，选中单本后才读取完整元数据证据，适配 3000 本以上书库。
...
| 格式 | 浏览器阅读方式 | 主要能力 |
| --- | --- | --- |
| PDF | PDF.js | 单页/双页、连续滚动、缩放、目录、页码跳转、搜索、浏览器即时朗读、屏幕常亮、文本高亮、书签/笔记、键盘操作 |
| EPUB | epub.js | 分页/连续滚动、字号、主题、目录、搜索、浏览器即时朗读、屏幕常亮、文本高亮、书签/笔记、键盘操作 |
| MOBI/AZW3 | 导入时生成 EPUB 阅读缓存 | 复用 EPUB 阅读能力，原文件仍被保留 |

- 每位用户独立保存阅读位置、整体进度、阅读状态和有效阅读时长；EPUB 翻页、PDF 换页后会延迟保存，并在退出阅读器、切换后台或页面卸载时立即写入，重开时按 EPUB CFI 或 PDF 页码恢复。
- 每位用户可在稳定的 PDF 页码或 EPUB CFI 位置添加、编辑、定位和删除私有书签、文本高亮及笔记；高亮支持五种颜色和可选批注，并可按单本书导出 Markdown 或 JSON。
- EPUB 当前章节和带文本层 PDF 当前页可直接调用浏览器 Web Speech API 朗读，支持设备音色选择、0.5–2 倍速、暂停、继续和停止；文字与音频不经过 PEUFMReader 服务端，音色是否离线由浏览器和操作系统决定。
- 阅读工具栏可开启“屏幕常亮”，设置仅保存在当前设备；它使用浏览器原生 Screen Wake Lock，不会让应用在后台保持运行。该功能要求 HTTPS 和较新的浏览器；iPhone/iPad 主屏幕安装版需要 iOS/iPadOS 18.4 或更高版本。
...
- 首页显示最近 7 天阅读时长、累计阅读时长、阅读中/已读完/收藏数量和可访问书库规模；“更多 → 阅读统计”提供独立统计中心。
- 阅读统计中心展示今天/7 天/30 天/累计时长、近 12 周热力图、14 天趋势、连续阅读、完成率、格式与题材分布和最近读完书籍。
- 书籍详情保存每位用户的阅读进度、状态、有效时长与最近阅读时间；热门书籍使用聚合阅读热度，不公开他人的私有笔记。
- 统计按当前用户可访问书籍过滤；阅读目标、提醒和数据导出仍列入后续 P2。
...
登录页和页面右上角的“界面主题”菜单可切换两套外观：

- **文学杂志**：纸白与钴蓝、衬线大标题、杂志式首页和完整比例书封。
- **深夜书房**：炭黑与青柠、书封层叠与柔和光影，书库、详情、统计和管理工作区使用统一深色配色。

默认使用文学杂志。选择仅保存到当前浏览器，同站点已打开的标签页同步切换；不会同步到其他设备或修改服务端账号配置。浏览器禁止存储时仍可在当前页面切换。界面主题与 EPUB 阅读器内的白天、护眼、夜间设置独立，不修改正文、阅读进度、收藏或导入逻辑。

没有封面的书籍使用本地文字封面，不访问外部图片服务。
...
- Web 性能：管理工作区和阅读器按需加载；首页与书库目录均采用集合权限查询；封面提供 240/320/480px WebP 响应式缩略图，长列表使用服务端分页和屏外渲染优化。
- 数据库：PostgreSQL 18。
...
诊断首页分区耗时时可筛选结构化日志；`summary`、`categories`、`hot` 和 `recommendations` 会分别记录：

```sh
docker compose logs --since 5m app | grep 'home section loaded'
```
...
- 书库权限默认允许，以兼容现有安装。管理后台可建立“书库组”和“用户组”：公开书库组默认可见，受限书库组默认隐藏，再按用户组批量允许或拒绝；一本书可以属于多个书库组。
- 权限优先级为“管理员 → 单书用户规则 → 用户组拒绝 → 用户组允许 → 书库组默认值”。单书“允许”“拒绝”或“恢复默认”仍可用于个别例外。
- 同一权限判定覆盖网页目录、首页、收藏、推荐、详情、文件内容、封面、阅读会话、OPDS 与设备进度接口，避免通过其他入口旁路访问。
...
网页“批量导入”会保存每一次导入报告，逐项记录新增、重复和失败原因；同一文件会按 SHA-256 识别为重复并跳过，不会新增第二本书。删除报告只清理报告和任务明细，不会删除书籍。详见[批量导入历史与报告](docs/validation/import-history-reports.md)。

上传前仅发送文件大小进行批量预检查；服务器存在同大小托管文件，或本次选择了多个同大小文件时，浏览器才读取本地文件计算完整 SHA-256。计算由单个后台 Worker 以 2 MiB 分块完成，显示本地校验进度，不上传正文、不缓存整本书，并支持普通 HTTP 的 NAS 地址。确认重复后直接跳过上传，报告显示跳过结果与本次节省的上传量。同名或同大小不代表内容相同，不会单独作为跳过依据。Calibre 只读引用尚未存储内容哈希，不参与此精确检查。

上传过程临时使用容器 `/tmp/multipart-*` 和 `/data/staging/upload-*.part`（后者对应 `${PEUFM_DATA_ROOT}/staging`）。正常请求结束后自动清理；跨文件系统复制入库后也会删除暂存源文件。封面、OCR、Kindle 转换文件位于独立的 `cache` 目录，是已有书籍的共享阅读资源，不应当作重复上传垃圾删除。进程被强制终止仍可能遗留暂存文件；手动清理前应确认文件未被进程使用，并与托管副本逐字节比对，只删除确认重复的暂存文件。
...
docker run --rm -v "$PWD/server:/src" -w /src \
  golang:1.26.6-bookworm /usr/local/go/bin/go test ./...
```
...
- 离线书籍使用浏览器站点存储，可能受系统空间策略清理；退出登录会删除当前账号在该设备上的书籍副本。离线期间书签和高亮暂不编辑，重新联网后恢复。
