...
const readingMarkColumns = `id,book_file_id,kind,position,overall_progress,label,body,created_at,updated_at`
...
func scanReadingMark(row readingMarkScanner) (ReadingMark, error) {
	...
	err := row.Scan(&mark.ID, &mark.BookFileID, &mark.Kind, &mark.Position, &mark.OverallProgress, &mark.Label, &mark.Body, &mark.CreatedAt, &mark.UpdatedAt)
	return mark, err
}
...
func (s *Store) SaveReadingMark(ctx context.Context, userID, bookFileID int64, kind string, position json.RawMessage, progress float64, label, body string) (ReadingMark, error) {
	mark, err := scanReadingMark(s.pool.QueryRow(ctx, `
		INSERT INTO reading_marks(user_id,book_file_id,kind,position,overall_progress,label,body)
		...
		RETURNING `+readingMarkColumns, userID, bookFileID, kind, position, progress, label, body))
	if err != nil {
...
func (s *Store) UpdateReadingMark(ctx context.Context, userID, markID int64, label, body string) (ReadingMark, bool, error) {
	mark, err := scanReadingMark(s.pool.QueryRow(ctx, `UPDATE reading_marks SET label=$1,body=$2,updated_at=now()
		WHERE id=$3 AND user_id=$4 RETURNING `+readingMarkColumns, label, body, markID, userID))
	if errors.Is(err, pgx.ErrNoRows) {
		return ReadingMark{}, false, nil
	}
	if err != nil {
